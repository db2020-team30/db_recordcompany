...
def choose(a,conn): # επιλογή πίνακα(για εισαγωγή, διαγραφή και αλλαγή στοιχειων) ή επιλογή άλμπουμ μεσα από combobox(για τα υπόλοιπα)
    if(a<6):
...
def homescreen(conn): #δημιουργία αρχικής οθόνης
    ...
    #δημιουργία κουμπιών
    btn1=HoverBtn(main,bg='white',width=500, activebackground='light cyan', relief='flat',text="Φυσικές πωλήσεις",font=('helvetica','14'),command=lambda: sales.cd_or_vinyl(conn))
    ...
    btn4=HoverBtn(main,bg='white',width=500, activebackground='light cyan', relief='flat',text="Συγκροτήματα/Τραγουδιστές",font=('helvetica','14'),command=lambda: musicians.choice(2,conn))
    btn5=HoverBtn(main,bg='white',width=500, activebackground='light cyan', relief='flat',text="Στοιχεία συμβολαίων ανά κατηγορία καλλιτεχνών",font=('helvetica','14'),command=lambda: musicians.choice(1,conn))
    ...
    btn7=HoverBtn(main,bg='white',width=500, activebackground='light cyan', relief='flat',text="Πελάτες και ποσότητες των προϊόντων που προμηθεύτηκαν",font=('helvetica','14'),command=lambda: partners.pelates(conn))
    btn8=HoverBtn(main,bg='white',width=500, activebackground='light cyan', relief='flat',text="Γραφίστες και τα άλμπουμ που έχουν επιμεληθεί",font=('helvetica','14'),command=lambda: partners.grafistas(conn))
    btn9=HoverBtn(main,bg='white',width=500, activebackground='light cyan', relief='flat',text="Άλμπουμ ανά μουσικό είδος",font=('helvetica','14'),command=lambda: album_info.album_per_genre(conn))
    btn10=HoverBtn(main,bg='white',width=500, activebackground='light cyan', relief='flat',text="Άλμπουμ ανά studio ηχογράφησης",font=('helvetica','14'),command=lambda: partners.album_per_studio(conn))
...
def connect(_event=None): #δημιουργία σύνδεσης
    ...
    u=e1.get()
    p=e2.get()
    if (u>'\u0080' and u<'\u03ce') or (p>'\u0080' and p<'\u03ce'): #η περίπτωση που οι εισοδοι δεν αποτελούνται μόνο απο λατινικούς χαρακτήρες
        if(errors!=1):
